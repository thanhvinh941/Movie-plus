...
public class MovieBannerInternalController {
	...
	public String insertMovieTrailer(@RequestBody String requestStr) {
		CreateInternalApiRequest<MovieBanner> request = new CreateInternalApiRequest<MovieBanner>();
		// DecodeRequest
		try {
			request = new ObjectMapper().readValue(requestStr, new TypeReference<CreateInternalApiRequest<MovieBanner>>() {});
		} catch (Exception e) {
